...
void XML_response(Print& dest)
{
  ...
  dest.printf_P(PSTR("<ns>%d</ns><nr>%d</nr><nl>%d</nl><nf>%d</nf><nd>%d</nd><nt>%d</nt><fx>%d</fx><sx>%d</sx><ix>%d</ix><fp>%d</fp><wv>%d</wv><ws>%d</ws><ps>%d</ps><cy>%d</cy><ds>%s%s</ds><ss>%d</ss>"),
  ...
  // Song playback status: current file, playback state, and /songs FS usage.
  dest.print(F("<sng>"));
  dest.print(song.currentFile().length() ? song.currentFile() : F("none"));
  dest.print(F("</sng><scmd>"));
  dest.print(song.stateString());
  dest.print(F("</scmd><sfs>"));
  dest.printf_P(PSTR("%u/%uKB"), (unsigned)(song.fsUsedBytes() / 1024), (unsigned)(song.fsTotalBytes() / 1024));
  dest.print(F("</sfs>"));
