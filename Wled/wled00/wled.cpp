...
void WLED::loop()
{
  ...
  userLoop();
  UsermodManager::loop();
  song.tick(); // non-blocking .bin song playback: JSON packets -> WLED internal JSON parser, no UART
  #ifdef WLED_DEBUG
...
void WLED::setup()
{
  ...
  DEBUG_PRINTLN(F("Song system setup"));
  song.begin(); // storage + playback engine + /song web UI, no UART
  DEBUG_PRINTF_P(PSTR("heap %u\n"), getFreeHeapSize());
