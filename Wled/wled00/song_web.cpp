...
<br>
<select id="songList" size="8" style="width:100%"></select>
<br><br>
...
function refresh(){
  fetch('/songs/list').then(r=>r.json()).then(d=>{
    ...
    d.files.forEach(function(f){var o=document.createElement('option');o.value=f;o.textContent=f;if(f===d.current)o.selected=true;l.appendChild(o);});
