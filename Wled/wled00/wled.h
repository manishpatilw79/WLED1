...
// Song playback controller (JSON-only .bin playback, see song.h).
// Included here (after all core WLED types/globals are declared) so that
// WLED::setup()/WLED::loop() can call song.begin()/song.tick() directly.
#include "song.h"
