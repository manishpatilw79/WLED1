...
// Ensures /songs exists on LittleFS and registers the /song web UI plus the
// /songs/* endpoints (list/play/pause/resume/stop/delete/upload) on WLED's
// global AsyncWebServer instance. Called once from Song::begin() (song.cpp).
void initSongUpload() {
  songStorage.begin();
  registerSongEndpoints(server);
}
